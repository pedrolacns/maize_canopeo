...
const capturedMeasures = document.getElementById('capturedMeasures');
const measurements = [];
...
captureButton.addEventListener('click', () => {
    ...
    currentImage = canvas;
    const canopyCover = processImage(canvas);
    
    // Add the measurement to the list and array
    measurementCount++;
    const listItem = document.createElement('li');
    listItem.textContent = `Medida ${measurementCount}: ${canopyCover}%`;
    capturedMeasures.appendChild(listItem);
    measurements.push({count: measurementCount, value: canopyCover});
});
...
// Function to export measurements to CSV
function exportToCSV() {
    let csvContent = "data:text/csv;charset=utf-8,";
    csvContent += "Medida,Valor de Canopeo\n";
    measurements.forEach(function(measurement) {
        csvContent += `${measurement.count},${measurement.value}\n`;
    });

    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", "medidas_canopeo.csv");
...
document.querySelector('.buttons').appendChild(exportButton);
